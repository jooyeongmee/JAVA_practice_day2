...
//int[] arr = new int[5]{1,2,3,4,5}; 배열 크기를 지정하여 생성하는 것과 리스트로 초기화를 하는 것을 동시에 할 수 없다.
//int arr[5]; 배열을 선언하려면 int arr[]이고, 크기를 지정하여 초기화 하려면 int arr[] = new int[5]; 이다.
...
//5-2. 다음과 같은 배열이 있을 때, arr[3].length의 값은? // 2이다.
class Exercise5_2 {
    public static void main(String[] args) {
        int[][]arr ={
                {5,5,5,5,5},
...
//        System.out.print(Arrays.toString(ballArr));
